...
import com.wear.me.config.KafkaProperties;
...
class WearMeKafkaResourceIT {
    ...
    void setup() {
        ...
        WearMeKafkaResource kafkaResource = new WearMeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/wear-me-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/wear-me-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
